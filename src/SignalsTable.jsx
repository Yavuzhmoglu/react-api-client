...
export default function SignalsTable() {
    ...
    const [selectedRange, setSelectedRange] = useState("1y"); // motorun sıkışma/CMF pencereleri için yeterli geçmiş gerekiyor
    ...
    const fetchSignals = async () => {
        try {
            setLoading(true);
            // Not: backend artık "interval"/"backtest" parametrelerini kullanmıyor -- canlı motor
            // her zaman günlük (1d) mumlarla, seçilen "range" kadar geçmişle çalışıyor.
            const url = `https://dotnet-api-bist.onrender.com/api/WhaleSignals?symbols=${latestSymbols.current}&range=${latestRange.current}`;
            // const url = `http://localhost:10000/api/WhaleSignals?symbols=${latestSymbols.current}&range=${latestRange.current}`;
    ...
    <Card title="🐋 Yavuz Intel - Balina Sinyalleri" className="shadow-lg">
      ...
      <div className="overflow-x-auto">
        ...
        >
          ...
          />
          <Column field="confidence" header="Güven %" sortable filter />
          <Column field="score" header="Anomali Skoru (0-100)" sortable filter />
